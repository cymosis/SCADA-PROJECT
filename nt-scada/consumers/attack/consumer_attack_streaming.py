"""
Enhanced Streaming Consumer for Attack Data
...
KAFKA_GROUP_ID = 'attack-streaming-consumer'
...
STATS_BUCKET = 'attack_streaming_stats'
...
def write_streaming_stats(write_api, stats_tracker, timestamp, org):
    ...
    for sensor in SENSOR_COLUMNS:
        ...
        if stats and stats['count'] > 0:
            point = Point("attack_streaming_stats") \
                .tag("sensor", sensor) \
...
def ensure_stats_bucket(client, org):
    """Ensure attack_streaming_stats bucket exists"""
    try:
...
def main():
    print(f"\n{'='*80}")
    print(f"STREAMING-ENHANCED ATTACK DATA CONSUMER")
    ...
    try:
        for msg in consumer:
            ...
            if message_count % stats_write_interval == 0:
                ...
                print(f"⚡ {message_count} attack messages processed")
        ...
        print(f"{'='*80}")
        print(f"Total attack messages: {message_count}")
        print(f"\nTop 5 Sensors:")
